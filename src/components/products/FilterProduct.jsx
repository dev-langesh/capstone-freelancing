...
export default function FilterProduct({ setProducts }) {
    ...
    try {
      ...
      if (category) {
        const categoryResponse = await axios.post(
          `${import.meta.env.VITE_APP_API_URL}/products/category`,
          { category }
        );
      ...
      if (brand) {
        const brandResponse = await axios.post(
          `${import.meta.env.VITE_APP_API_URL}/products/brand`,
          { brand }
        );
  ...
  async function clearFilter() {
    const api = `${import.meta.env.VITE_APP_API_URL}/products/active`;
